...
static NSUInteger maxSentenceLength = 7;

@interface MVTextGenerator ()
@property (strong, nonatomic) NSArray *loremWords;
@end

@implementation MVTextGenerator
-(instancetype)init {
    if (self = [super init]) {
        _loremWords = [[@"lorem,ipsum,dolor,sit,amet,consectetuer,adipiscing,elit,integer,in,mi,a,mauris,"\
...
- (NSString *)randomWord {
    return [self.loremWords objectAtIndex:arc4random_uniform((uint32_t)self.loremWords.count)];
}

- (NSString *)words:(NSUInteger)count {
    NSMutableString *mutableString = [NSMutableString new];
    for (int i = 0; i<count; i++) {
        [mutableString appendString:[self randomWord]];
        if (i != count-1) {
...
- (NSString *)sentences:(NSUInteger)count {
    NSMutableString *mutableString = [NSMutableString new];
    for (int i = 0; i<count; i++) {
        [mutableString appendString:[self words:maxSentenceLength]];
        [mutableString appendString:@"."];
        if (i != count-1) {
            [mutableString appendString:@" "];
        }
